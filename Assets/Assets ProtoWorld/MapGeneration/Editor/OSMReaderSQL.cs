...
public class OSMReaderSQL : Editor
{
    ...
    [MenuItem("Gapslabs Extended Editor/Load OSM Data/Generate Roads and Buildings", false, 1)]
    static void Create2ServerSide()
    {
    ...
    [MenuItem("Gapslabs Extended Editor/Load OSM Data/Generate Buildings only", false, 1)]
    static void Create2_2()
    {
        var globalO = GameObject.Find("AramGISBoundaries");
        var mapproperties = globalO.GetComponent<MapBoundaries>();
    ...
    [MenuItem("Gapslabs Extended Editor/Load OSM Data/Generate Roads only", false, 1)]
    static void Create2_3()
    {
    ...
    [MenuItem("Gapslabs Extended Editor/Load OSM Data/Assign static layers")]
    public static void AssignStaticLayers()
    {
    ...
    [MenuItem("Gapslabs Extended Editor/Load OSM Data/Get Traffic lights from the database")]
    ...
    [MenuItem("Gapslabs Extended Editor/Load OSM Data/Get Water areas from the database")]
    static void CreateWaterAreas()
    {
    ...
    [MenuItem("Gapslabs Extended Editor/Clean Scene")]
    ...
    //[MenuItem("Gapslabs Extended Editor/Load OSM Data/Bus stops from the database")]
    ...
    //[MenuItem("Gapslabs Extended Editor/Load OSM Data/Metadata from the database")]
    //public static void AddBusStopLogic()
    //{
    //    ChooseConnection();
    //    ServiceGapslabsClient client = ServicePropertiesClass.GetGapslabsService(ServicePropertiesClass.ServiceUri);
    //    var busStops = GameObject.FindGameObjectsWithTag("BusStop");
    //    for (int i = 0; i < busStops.Length; i++)
    //    {
    //        try
    //        {
    //            if (!EditorUtility.DisplayCancelableProgressBar("Adding bus stop logic...", i + " of " + busStops.Length, i / (float)busStops.Length))
    //            {
    //                var tempBusStop = busStops[i].GetComponent<BusStopClass>();
    //                if (tempBusStop == null)
    //                    tempBusStop = busStops[i].AddComponent<BusStopClass>();
    //                var id = ObjectInformerGUI.ParseGameobjectName(busStops[i].name);
    //                if (id == "")
    //                    continue;
    //                tempBusStop.Id = id;
    //                Debug.Log("Doing node " + id);
    //                var bus = client.GetNodeInfo(id, wcfCon);
    //                tempBusStop.Latitude = bus.lat;
    //                tempBusStop.Longitude = bus.lon;
    //                var tags = client.GetNodeTags(id, wcfCon);
    //                var tname = tags.Where(t => t.KeyValue[0].ToLower() == "name");
    //                var tzone = tags.Where(t => t.KeyValue[0].ToLower() == "zone");
    //                var tnetworkOrOperator = tags.Where(t => t.KeyValue[0].ToLower() == "operator" || t.KeyValue[0].ToLower() == "network");
    //                var troute_ref = tags.Where(t => t.KeyValue[0].ToLower() == "route_ref");
    //                var tlocal_ref = tags.Where(t => t.KeyValue[0].ToLower() == "local_ref");
    //                var tref = tags.Where(t => t.KeyValue[0].ToLower() == "ref");
    //                var tshelter = tags.Where(t => t.KeyValue[0].ToLower() == "shelter");
    //                var twheelchair = tags.Where(t => t.KeyValue[0].ToLower() == "wheelchair");
    //                var tbench = tags.Where(t => t.KeyValue[0].ToLower() == "bench");

    //                if (tname.Count() != 0)
    //                    tempBusStop.Name = tname.First().KeyValue[1];
    //                if (tzone.Count() != 0)
    //                    tempBusStop.Zone = tzone.First().KeyValue[1];
    //                if (tnetworkOrOperator.Count() != 0)
    //                    tempBusStop.NetworkOrOperator = tnetworkOrOperator.First().KeyValue[1];
    //                if (troute_ref.Count() != 0)
    //                    tempBusStop.RouteReference = troute_ref.First().KeyValue[1].Split(new string[] { ";" }, System.StringSplitOptions.RemoveEmptyEntries);
    //                if (tlocal_ref.Count() != 0)
    //                    tempBusStop.SetLocalReference(tlocal_ref.First().KeyValue[1]);
    //                if (tref.Count() != 0)
    //                    tempBusStop.SetReference(tref.First().KeyValue[1]);
    //                if (tshelter.Count() != 0)
    //                    tempBusStop.Shelter = tshelter.First().KeyValue[1].ToLower() == "yes" ? true : false;
    //                if (twheelchair.Count() != 0)
    //                    tempBusStop.Wheelchair = twheelchair.First().KeyValue[1].ToLower() == "yes" ? true : false;
    //                if (tbench.Count() != 0)
    //                    tempBusStop.Bench = tbench.First().KeyValue[1].ToLower() == "yes" ? true : false;
    //            }
    //            else
    //                EditorUtility.ClearProgressBar();
    //        }
    ...
    //[MenuItem("Gapslabs Extended Editor/Load OSM Data/Generate Roads + Building Shapes (Basic)", false, 1)] -- Process not optimized
    static void Create2()
    {
        var globalO = GameObject.Find("AramGISBoundaries");
        var mapproperties = globalO.GetComponent<MapBoundaries>();
    ...
    static void ImportOSMDataSQL_Ext(bool GenerateBuildingShapes, bool GenerateRoads, bool GenerateBuildings)
    {
        ...
        {
            if (!EditorUtility.DisplayCancelableProgressBar("Importing data", "Generating roads and buildings\t" + progress + "/" + totalWays, progress++ / (float)totalWays))
            {
