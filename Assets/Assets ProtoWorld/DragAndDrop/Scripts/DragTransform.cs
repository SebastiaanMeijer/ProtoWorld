...
class DragTransform : MonoBehaviour
{
    ...
    void Start()
    {
        //change the layer of the gameObject to 14 which is droppedObjects layer
        gameObject.layer = 14;

        //change the layer of all the childs of this object to 14 also
        for (int i = 0; i < this.transform.childCount; i++)
        {
            this.transform.GetChild(i).GetComponent<Transform>().gameObject.layer = 14;
        }
    ...
    {
        if (multitouch != null)
            multitouch.movingDragAndDropObject = value;
    }
}