...
python "%SUMO_HOME%\tools\osmGet.py" --bbox="%~1","%~2","%~3","%~4" --prefix=map --output-dir="%~6" 2>> "%~6"/map.log
...
python "%SUMO_HOME%\tools\osmBuild.py" --prefix=map --osm-file="%~6"/map_bbox.osm.xml --tiles=1 --vehicle-classes=road --output-directory="%~6" 2>> "%~6"/map.log
...
python "%SUMO_HOME%\tools\randomTrips.py" --net-file="%~6"/map.net.xml --begin=0 --end=5000  --prefix=veh --period="%~5" --vclass=passenger --intermediate=300 --output-trip-file="%~6"/map.trips.xml 2>> "%~6"/map.log
